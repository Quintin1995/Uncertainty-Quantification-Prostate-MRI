...
# """
# ================================================================================
# Uncertainty–Error Correlation Extraction for Prostate MRI
# ================================================================================
...
# 🎯 Scientific Purpose
# --------------------------------------------------------------------------------
# To evaluate whether uncertainty maps generated by two different methods—
# Gaussian Monte Carlo (MC) and structured echo-train dropout (LXO)—correlate 
# with actual reconstruction error. Metrics are computed:
#     • Per-slice
#     • Per-region (whole slice, prostate, lesion)
#     • Per-acceleration factor (R=3, R=6)
#     • Per-method (Gaussian vs LXO)

# This supports reproducible, region-stratified correlation analysis of UQ quality.

# --------------------------------------------------------------------------------
# 📊 Metrics Stored Per Region (and Slice)
# --------------------------------------------------------------------------------
# | Metric              | Description                                       |
# |---------------------|---------------------------------------------------|
...
# slice × region × acceleration × UQ method.

# --------------------------------------------------------------------------------
# 🔁 Looping Structure (5 Levels)
# --------------------------------------------------------------------------------
...
# Images themselves are not stored—only derived statistics.
...
def compute_slice_level_stats(
    ...
    # LOGGER.info(f"📐 Shape of reference image: {r1_arr.shape}")
        ...
        for i in range(r1_arr.shape[0]):
            ...
            for region_name, mask in regions.items():
                if mask is not None and not np.any(mask):
                    LOGGER.warning(f"    ⚠️ Empty mask for region '{region_name}' in slice {i}")
                    continue
                ...
                if voxelwise_error.size < 2 or voxelwise_uq_cv.size < 2:
                    LOGGER.warning(f"    ⛔ Insufficient data in slice {i}, region '{region_name}'")
                    continue
                ...
                LOGGER.info(f"    ✅ Slice {i:02d}, Region: {region_name:8s} | mean_abs: {metrics['mean_abs']:.4f}, "
                        f"mean_uq(CV): {metrics['mean_uq']:.4f}, Pearson: {metrics['pearson_corr']:.3f}, spearman_corr: {metrics['spearman_corr']:.3f}")

    LOGGER.info(f"📊 Total rows for {pat_id}: {len(all_rows)}")
    return all_rows
...
def process_patients_and_store_stats(
    ...
    for idx, pat_id in enumerate(pat_ids):
        if patient_exists_in_table(db_fpath, table_name, pat_id, uq_method):
            LOGGER.info(f"⏭️  {idx + 1}/{len(pat_ids)} Skipping patient {pat_id} (already in table for '{uq_method}')")
            skipped_count += 1
            continue
    ...
    if not all_stats:
        LOGGER.warning("⚠️ No new data computed. Exiting early.")
        return

    long_df = pd.DataFrame(all_stats)

    LOGGER.info(f"📥 Storing {processed_count} new patients in database ({skipped_count} skipped).")
    with sqlite3.connect(str(db_fpath)) as conn:
        long_df.to_sql(table_name, conn, if_exists="append", index=False)
    LOGGER.info(f"✅ Appended to table '{table_name}' in {db_fpath}")
...
def create_table_if_not_exists(db_fpath: Path, table_name: str):
    ...
    if not db_fpath.exists():
        LOGGER.info(f"📂 Creating new database file at {db_fpath}")
    ...
    try:
        ...
        LOGGER.info(f"✅ Verified/created table '{table_name}' in {db_fpath.name}")
...
def patient_exists_in_table(
    ...
    try:
        ...
        LOGGER.error(f"❌ Error checking patient {pat_id} in table {table_name}: {e}")
        return False
...
if __name__ == '__main__':
    ...
    # === Optional: Drop Table ===
    if debug:
        LOGGER.info(f"🧨 Dropping table '{tablename}' if it exists...")
        with sqlite3.connect(str(db_fpath)) as conn:
            cursor = conn.cursor()
            cursor.execute(f"DROP TABLE IF EXISTS {tablename}")
            conn.commit()
        LOGGER.info(f"✅ Table '{tablename}' dropped successfully.")
    ...
    for u_method in uq_methods:
        ...
        LOGGER.info(f"📁 Data Roots Initialized for {u_method}")
