...
def create_umcg_lst_file(
    ...
    institute = 'umcg',
    splitsize: int = None,
    inclusion_list: List[str] = None
):
    pats_dir = rootdir / 'data'
    
    # Step 1 - Sorting: Sort on first number of pat_ids: 0001_ANONxxxxxxx
    ...
    # Step 3 - Inclusion: include only patients from the inclusion list
    print(f"Number of patients before inclusion: {len(pat_dirs)}")
    if inclusion_list is not None:
        pat_dirs = [d for d in pat_dirs if d.name.split('_')[0] in inclusion_list]
    print(f"Number of patients after inclusion: {len(pat_dirs)}")
        
    # Step 4 - Create a list of .h5 files
    h5_paths = []
    ...
    # Step 5 - Split the list in smaller lists of size splitsize and write to file
    if splitsize is not None:
        Path(f"split_by_{splitsize}").mkdir(parents=True, exist_ok=True)
...
if __name__ == "__main__":
    ...
    with open('lists/inclusion/include_ids_sorted.lst', 'r') as f:
        include_ids = f.readlines()
    include_ids = [x.strip().split('_')[0] for x in include_ids]

    create_umcg_lst_file(
        rootdir        = Path('/scratch/p290820/datasets/003_umcg_pst_ksps'),
        # excl_list      = excl1+excl2,
        splitsize      = 20,
        inclusion_list = include_ids)
