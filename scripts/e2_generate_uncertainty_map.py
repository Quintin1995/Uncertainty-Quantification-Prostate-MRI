...
def generate_uncertainty_map(
    ...
    for idx, gaus_h5_file in enumerate(h5_files):
        print(f'\tProcessing: ({idx+1}/{len(h5_files)}) {gaus_h5_file.name}, with gaussian_id: {str(gaus_h5_file.stem).split("_")[-1]} ')
    ...
    reconstruction_stack_4d = np.stack(recons)
    print(f"\tShape of recons_4d: {reconstruction_stack_4d.shape}") if debug else None

    # 5) Calculate uncertainty map
    uq_map = calculate_uncertainty_map(reconstruction_stack_4d, debug=True)
    
    # 6) Round the uncertainty map if required
...
if __name__ == "__main__":
    ...
    pat_ids = [
        ...
        '0077_ANON8634437',
        '0078_ANON6883869',
        '0079_ANON8828023',     # this one has been run.
        # '0080_ANON4499321',
        # '0081_ANON9763928',
        # '0082_ANON6073234',
        # '0083_ANON9898497',
